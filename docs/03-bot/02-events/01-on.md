...
⚠ توجه: بسته به نوع رویداد (`"update"` , `"inline"` , `"error"`)، آرگومان ctx می‌تونه یک [Update](/docs/bot/contexts/update) , [Inline](/docs/bot/contexts/inline) یا `string` باشه.
...
bot.on<StoreType, "update">("update", async (ctx) => {
  console.log(ctx.store.group);
});
